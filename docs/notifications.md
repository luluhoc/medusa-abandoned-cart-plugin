...
```ts
{
  ...
  template: "d-1a2b3c…",            // the stage's template for this cart's locale
  data:     { /* below */ },
}
```
...
```jsonc
{
  ...
  "locale": "fr-CA",              // resolved locale, or null
...
2. `localeData` for this cart's locale,
3. the stage's own `data`,
4. the stage's `localeData` for this cart's locale,
5. everything above.

So later entries override earlier ones, and none of them can shadow the built-in fields.
...
- **`locale` and `language` are `null`** unless the cart resolves to one. The plugin never formats
  anything with them — pass them, with `currency_code`, to `Intl.NumberFormat` yourself.
...
A stage can carry one template id per locale, and the plugin picks the one matching the cart:

```ts
stages: [
  { id: "reminder", delay: "4h", template: "d-reminder-en",
    templates: { fr: "d-reminder-fr", de: "d-reminder-de" } },
]
```

Resolution order is the stage's `templates`, the top-level `templates`, `templatePattern`, then the
stage's plain `template` — so an untranslated locale still gets an email in the default language.
[Localization](./localization.md) covers the whole picture: how a cart's locale is decided,
per-locale data, and localized recovery links.
...
| `locale` | The locale this attempt was rendered in, `null` when none resolved. |
